...
class ChatMetadata(BaseModel):
    ...
    quality_threshold: Optional[float] = Field(
        0.7, description="Minimum quality score for results"
    )
...
class ChatQuery(BaseModel):
    ...
    query: str = Field(
        ..., min_length=1, max_length=1000, description="User's question or query"
    )
    metadata: Optional[ChatMetadata] = Field(
        None, description="Optional query metadata"
    )
...
class ChatResponse(BaseModel):
    ...
    processing_time: float = Field(
        ..., description="Time taken to process query (seconds)"
    )
