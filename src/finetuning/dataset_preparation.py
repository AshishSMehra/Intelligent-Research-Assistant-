...
class DatasetPreparation:
    ...
    def _generate_task_examples(
        self, text: str, metadata: Dict[str, Any], task_type: str, num_examples: int
    ) -> List[InstructionExample]:
        """
    ...
    def _split_text_for_tasks(
        self, text: str, task_type: str, num_chunks: int
    ) -> List[str]:
    ...
    def get_dataset_stats(self, dataset_dict: DatasetDict) -> Dict[str, Any]:
        ...
        for split_name, dataset in dataset_dict.items():
            split_stats = {
                ...
                "avg_instruction_length": sum(len(ex["instruction"]) for ex in dataset)
                / len(dataset),
                "avg_input_length": sum(len(ex["input"]) for ex in dataset)
                / len(dataset),
                "avg_output_length": sum(len(ex["output"]) for ex in dataset)
                / len(dataset),
