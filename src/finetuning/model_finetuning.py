...
class ModelFineTuning:
        ...
        if use_qlora and load_in_4bit:
            ...
            if qlora_config["load_in_4bit"]:
                try:
                    self.model = AutoModelForCausalLM.from_pretrained(
                        ...
                        quantization_config=bnb.BitsAndBytesConfig(
                            ...
                            bnb_4bit_compute_dtype=qlora_config[
                                "bnb_4bit_compute_dtype"
                            ],
                            bnb_4bit_use_double_quant=qlora_config[
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    model_name,
                    torch_dtype=(
                        torch.float16 if self.device.type == "mps" else torch.bfloat16
                    ),
                    device_map="auto" if self.device.type == "cuda" else None,
                )
        else:
    ...
    def prepare_dataset(
        self, dataset_dict: DatasetDict, max_length: int = 1024
    ) -> DatasetDict:
        """
    ...
    def create_trainer(
        self, train_dataset, eval_dataset, training_config: TrainingConfig
    ) -> Trainer:
        """
        ...
        training_args = TrainingArguments(
            ...
            dataloader_pin_memory=self.gpu_config.optimization_config[
                "dataloader_pin_memory"
            ],
            dataloader_num_workers=self.gpu_config.optimization_config[
    ...
    def train(self, training_config: TrainingConfig) -> Dict[str, Any]:
        ...
        tokenized_datasets = self.prepare_dataset(
            dataset_dict, max_length=training_config.max_length
        )
    ...
    def merge_and_save_model(
        self, base_model_name: str, adapter_path: str, output_path: str
    ) -> None:
        """
