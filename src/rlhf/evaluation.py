...
class RLHFEvaluator:
    ...
    def evaluate_factuality(self, prompt: str, response: str) -> float:
        ...
        if any(word in response.lower() for word in ["according to", "research shows", "study", "paper"]):
            score += 0.2
    ...
    def compute_text_metrics(self, predictions: List[str], references: List[str]) -> Dict[str, float]:
        ...
        if EVALUATE_AVAILABLE:
            ...
            if "bleu" in self.metrics:
                try:
                    bleu_score = self.metrics["bleu"].compute(predictions=predictions, references=references)
            ...
            if "rouge" in self.metrics:
                try:
                    rouge_score = self.metrics["rouge"].compute(predictions=predictions, references=references)
            ...
            if "bertscore" in self.metrics:
                try:
                    bert_score = self.metrics["bertscore"].compute(predictions=predictions, references=references, lang="en")
    ...
    def run_comprehensive_evaluation(self) -> Dict[str, Any]:
        ...
        # Compare models
        results["comparison"] = self._compare_models(results["baseline"], results["rlhf"])
    ...
    def _evaluate_model_responses(self, prompts: List[str], responses: List[str], model_name: str) -> Dict[str, Any]:
        ...
        reference_responses = [responses[i * self.config.num_samples] for i in range(len(prompts))]
    ...
    def _compare_models(self, baseline_results: Dict, rlhf_results: Dict) -> Dict[str, Any]:
    ...
    def _save_evaluation_results(self, results: Dict[str, Any]):
        ...
        results_file = os.path.join(self.config.output_dir, f"evaluation_results_{timestamp}.json")
    ...
    def _create_summary_report(self, results: Dict[str, Any], results_file: str):
        ...
        with open(report_file, 'w') as f:
            ...
            for metric in ["factuality", "helpfulness", "coherence"]:
                ...
                f.write(f"  Improvement: {comp['improvement']:.3f} ({comp['improvement_percent']:.1f}%)\n\n")
            
            f.write(f"Diversity:\n")
            f.write(f"  Baseline: {results['comparison']['diversity']['baseline']:.3f}\n")
            f.write(f"  RLHF: {results['comparison']['diversity']['rlhf']:.3f}\n")
            f.write(f"  Improvement: {results['comparison']['diversity']['improvement']:.3f}\n\n")
            ...
            improvements = []
            for metric in ["factuality", "helpfulness", "coherence"]:
                improvements.append(results["comparison"][metric]["improvement_percent"])
            
            avg_improvement = np.mean(improvements)
            
            if avg_improvement > 5:
                f.write("✅ RLHF training shows significant improvement across all metrics.\n")
            elif avg_improvement > 0:
                f.write("✅ RLHF training shows modest improvement.\n")
            else:
                f.write("⚠️ RLHF training did not show clear improvement. Consider adjusting parameters.\n")
    ...
    def run_human_evaluation(self, num_samples: int = 5) -> Dict[str, Any]:
        ...
        baseline_wins = sum(1 for result in evaluation_results if result["winner"] == "baseline")
        rlhf_wins = sum(1 for result in evaluation_results if result["winner"] == "rlhf")
        ...
        human_eval_results = {
            ...
            "rlhf_wins": rlhf_wins,
            "ties": ties,
            "baseline_win_rate": baseline_wins / len(evaluation_results) if evaluation_results else 0,
            "rlhf_win_rate": rlhf_wins / len(evaluation_results) if evaluation_results else 0,
