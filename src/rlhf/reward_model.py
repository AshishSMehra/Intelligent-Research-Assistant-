...
class RewardModel(nn.Module):
    ...
    def __init__(self, model_name: str, device: str = "auto"):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(
            model_name, num_labels=1  # Single scalar reward
        )
...
class RewardModelTrainer:
                    ...
                    preference_data.append(
                        {
                            ...
                            "rejected": rejected,
                            "chosen_rating": max(
                                response_i["rating"], response_j["rating"]
                            ),
                            "rejected_rating": min(
                                response_i["rating"], response_j["rating"]
                            ),
    ...
    def compute_pairwise_loss(
        self, chosen_rewards, rejected_rewards, margin: float = 0.1
    ):
        ...
        with torch.no_grad():
            for example in test_pairs:
                ...
                loss = self.compute_pairwise_loss(
                    torch.tensor([chosen_reward]), torch.tensor([rejected_reward])
                )
