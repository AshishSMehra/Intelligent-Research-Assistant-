...
"""
    ...
    def _generate_fallback_response(
        ...
        context_text = (
            top_chunk.get("text", "")[:200] + "..."
            if len(top_chunk.get("text", "")) > 200
            else top_chunk.get("text", "")
        )
        ...
        citations = [
            {
                ...
                "page": (
                    top_chunk.get("source_pages", [1])[0]
                    if top_chunk.get("source_pages")
                    else 1
                ),
                "text": (
                    top_chunk.get("text", "")[:100] + "..."
                    if len(top_chunk.get("text", "")) > 100
                    else top_chunk.get("text", "")
                ),
            }
        ]
