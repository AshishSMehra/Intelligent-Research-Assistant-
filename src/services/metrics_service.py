...
class MetricsService:
    ...
    def __init__(self):
        ...
        self.session_metrics = defaultdict(
            lambda: {"queries": 0, "total_tokens": 0, "total_time": 0.0, "errors": 0}
        )
    ...
    def log_response_time(self, endpoint: str, method: str, duration: float) -> None:
        ...
        if len(self.metrics["response_times"][key]) > 100:
            self.metrics["response_times"][key] = self.metrics["response_times"][key][
                -100:
            ]
    ...
    def log_session_activity(
        self, session_id: str, query_length: int, tokens_used: int, duration: float
    ) -> None:
        """
