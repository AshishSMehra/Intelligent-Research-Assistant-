...
class AgentOrchestrator:
    ...
    async def execute_workflow(self, query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            final_result = {
                ...
                "final_response": reasoning_result.data.get("generated_content", "") if reasoning_result.data else "",
                "sources": research_result.data.get("sources", []) if research_result.data else [],
                "metadata": {
            ...
            logger.info(f"Workflow {workflow_id} completed successfully in {execution_time:.3f}s")
    ...
    async def _execute_planning(self, query: str, context: Dict[str, Any] = None) -> AgentResult:
                    ...
                    },
                    {
                        ...
                        "parameters": {
                            "metrics": {
                                ...
                                "agents_used": ["planner", "research", "reasoner", "executor"]
                            },
                            "storage_type": "memory"
                        }
                    }
    ...
    async def execute_single_agent_task(self, agent_type: str, task: AgentTask) -> AgentResult:
        """
        ...
        if not agent.can_handle_task(task):
            raise ValueError(f"Agent {agent_type} cannot handle task type: {task.task_type}")
