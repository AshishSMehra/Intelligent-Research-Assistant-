...
class RateLimiter:
    ...
    def __init__(
        self, redis_url: str = "redis://localhost:6379", config: RateLimitConfig = None
    ):
        self.redis_client = redis.from_url(redis_url)
        self.config = config or RateLimitConfig()

        # Initialize rate limit windows
        self.windows = {"minute": 60, "hour": 3600, "day": 86400}
        ...
        for window_name, window_seconds in self.windows.items():
            ...
            if current_count >= limit:
                ...
                self.redis_client.setex(
                    penalty_key, penalty_duration, time.time() + penalty_duration
                )
...
class AbuseDetector:
    ...
    def detect_abuse(self, request) -> Tuple[bool, Dict[str, Any]]:
        ...
        if (
            ...
            and request.content_length > self.config.max_payload_size
        ):
...
class SecurityMiddleware:
    ...
    def __init__(
        self, rate_limiter: RateLimiter = None, abuse_detector: AbuseDetector = None
    ):
...
global_security_middleware = SecurityMiddleware(
    global_rate_limiter, global_abuse_detector
)
