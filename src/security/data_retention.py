...
class DataRetentionManager:
    ...
    def create_retention_record(self, user_id: str, data_category: DataCategory, 
                               data_id: str, created_date: datetime = None) -> str:
        ...
        if policy.retention_period == RetentionPeriod.INDEFINITE:
            ...
            scheduled_deletion_date = created_date + timedelta(seconds=policy.retention_period.value)
    ...
    def _store_retention_record(self, record: DataRetentionRecord):
        ...
        record_data = {
            ...
            "scheduled_deletion_date": record.scheduled_deletion_date.isoformat() if record.scheduled_deletion_date else "",
            "actual_deletion_date": record.actual_deletion_date.isoformat() if record.actual_deletion_date else "",
            "deletion_reason": record.deletion_reason or "",
            "archived": str(record.archived)
        }
        
        self.redis_client.hset(f"retention_record:{record.record_id}", mapping=record_data)
    
    def get_retention_records(self, user_id: str = None, data_category: DataCategory = None) -> List[DataRetentionRecord]:
    ...
    def schedule_data_deletion(self, user_id: str, data_category: DataCategory, 
                              deletion_date: datetime = None) -> bool:
        ...
        try:
            ...
            if deletion_date is None:
                if policy.retention_period == RetentionPeriod.INDEFINITE:
                    return False
                deletion_date = datetime.now() + timedelta(seconds=policy.retention_period.value)
    ...
    def execute_data_deletion(self, user_id: str, data_category: DataCategory) -> bool:
        ...
        try:
            logger.info(f"Executing data deletion for user {user_id}, category {data_category.value}")
            ...
            logger.info(f"Data deletion completed for user {user_id}, category {data_category.value}")
            return True
    ...
    def archive_data(self, user_id: str, data_category: DataCategory) -> bool:
        ...
        try:
            ...
            logger.info(f"Archiving data for user {user_id}, category {data_category.value}")
    ...
    def get_data_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        for category in DataCategory:
            ...
            category_summary = {
                ...
                "retention_period": policy.retention_period.value,
                "auto_delete": policy.auto_delete,
                "scheduled_deletions": len([r for r in records if r.scheduled_deletion_date and r.scheduled_deletion_date > datetime.now()])
            }
...
class OptOutManager:
    ...
    def create_opt_out_request(self, user_id: str, opt_out_type: OptOutType, 
                              reason: str = None, effective_date: datetime = None) -> str:
    ...
    def get_user_opt_outs(self, user_id: str) -> List[OptOutRequest]:
        ...
        return [request for request in self.opt_out_requests.values() if request.user_id == user_id]
    ...
    def is_opted_out(self, user_id: str, opt_out_type: OptOutType) -> bool:
        ...
        return any(opt_out.opt_out_type == opt_out_type and opt_out.status == "active" for opt_out in user_opt_outs)
...
def require_data_retention_check():
    ...
    def decorator(f):
        def decorated_function(*args, **kwargs):
            ...
            if user_id:
                if global_opt_out_manager.is_opted_out(user_id, OptOutType.DATA_COLLECTION):
                    return jsonify({
