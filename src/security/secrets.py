...
try:
    ...
    logger.warning(
        "HashiCorp Vault client not available. Install with: pip install hvac"
    )
    ...
    def list_secrets(self) -> list:
        ...
        try:
            response = self.s3_client.list_objects_v2(
                Bucket=self.bucket_name, Prefix="secrets/"
            )
...
class VaultManager(SecretsManager):
    ...
    def __init__(
        self, vault_url: str = None, token: str = None, mount_point: str = "secret"
    ):
        if not VAULT_AVAILABLE:
            raise ImportError(
                "HashiCorp Vault client (hvac) is required for Vault manager"
            )
    ...
    def decrypt(self, ciphertext: str, key_id: str = None) -> str:
        ...
        try:
            response = self.client.secrets.transit.decrypt_data(
                name=key_id or "default-key", ciphertext=ciphertext
            )
